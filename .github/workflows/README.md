...
| `_build.yml`                    | `workflow_call` (internal)          | Renders `Chromatyx.pdf` (the one shippable artifact) |
...
```
   ...
   dispatch ────┤       │  on success on main
        ...
        └────┬─────┘  └─────┬───────┘
             │              │
             ▼              ▼
        (both call _build.yml: nanDECK renders Chromatyx.pdf)
             │              │
             ▼              ▼
...
[nanDECK](https://www.nandeck.com) build (cached via `actions/cache`), enables
its batch mode through `nanDECK.ini` (`enable_batch=1` — CLI actions are
silently ignored without it), runs
...
|---------------------------------------|----------------------|
| `app-artifacts` (`Chromatyx.pdf`)     | release + nightly    |
| `Chromatyx-PrintAndPlay` (smoke PDF)  | ci.yml (windows leg) |
